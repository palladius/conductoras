...
class ArcadeHandler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        if parsed_path.path == "/api/repos":
            ...
            if os.path.exists(GIT_DIR):
                for d in os.listdir(GIT_DIR):
                    ...
                    # For now, list all git repos to be safe, but you can filter for conductor/ if needed.
                    # The user asked "only those with git/REPO/conductor/ folder?" but we also want to test media-arneis.
                    # Let's list ANY repo that is a git repo.
                    if os.path.isdir(full_path) and os.path.isdir(os.path.join(full_path, ".git")):
                        # Check if it has a conductor directory
                        has_conductor = os.path.isdir(os.path.join(full_path, "conductor"))
                        repos.append({"name": d, "has_conductor": has_conductor})
            
            # Sort with conductor repos first, then alphabetically
        ...
        # Fallback to serving static files from html/
        super().do_GET()

if __name__ == "__main__":
    # Change directory to html/ to serve static files correctly
    html_dir = os.path.join(os.path.dirname(__file__), "..", "html")
    os.chdir(html_dir)
    # Allow address reuse so we don't get "Address already in use" errors
